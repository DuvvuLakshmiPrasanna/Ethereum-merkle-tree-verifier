...
Interacts with Ethereum JSON-RPC endpoint to retrieve block and transaction data.
"""
...
def fetch_block(rpc_url: str, block_number: Union[int, str] = "latest") -> dict:
    ...
    if isinstance(block_number, int):
        ...
        block_param = block_number  # "latest", "earliest", "pending"
...
def fetch_transaction_proof(rpc_url: str, tx_hash: str) -> dict:
    ...
    Note: eth_getProof works for state/storage; transaction inclusion is
    proven via the Merkle tree over block transactions.
    This fetches the receipt which confirms inclusion.
    """
    payload = {
        "jsonrpc": "2.0",
...
def inspect_block(block: dict) -> None:
    ...
    if tx_count > 0:
        print(f"\n  First Transaction Hash : {block['transactions'][0].get('hash', 'N/A')}")
        if tx_count > 1:
            print(f"  Last  Transaction Hash : {block['transactions'][-1].get('hash', 'N/A')}")
...
if __name__ == "__main__":
    ...
    if not rpc_url:
        raise EnvironmentError(
            ...
            "Please set it to your Alchemy or Infura endpoint."
        )

    block = run_fetch_demo(rpc_url)
    print(f"\nBlock fetched successfully with {len(block.get('transactions', []))} transactions.")
